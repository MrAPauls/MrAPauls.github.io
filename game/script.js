...
function updateNumberHintButtonLabel() {
  ...
  numberHintButton.textContent = numberHintVisible ? "Hide Number Hint" : "Show Number Hint";
}
...
function applyNumberHintVisibility() {
  ...
  for (const tile of tiles) {
    if (!tile.classList.contains("path") || !tile.classList.contains("revealed")) {
      tile.textContent = "";
      continue;
    }

    tile.textContent = numberHintVisible ? String(getOrthogonalPathNeighborCount(tile)) : "";
  }
}
